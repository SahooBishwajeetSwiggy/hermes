...
from src.solver import expand_vehicle_types
from src.utils import format_time_window, minutes_to_time_str
...
def print_solution(data, routing, manager, solution):
    ...
    for vehicle_id in range(len(vehicle_type_list)):
        ...
        while not routing.IsEnd(index):
            ...
            if node_index != load_config()["depot_index"]:
                delivery_points += 1
        ...
        if "time_windows" in data and load_config()["enable_time_windows"]:
            time_dim = routing.GetDimensionOrDie("Time")
            route_with_times = []
            for node in route:
                time_window = data["time_windows"][node]
                # location_str = f"{data["locations"][node]} ({time_window[0]} - {time_window[1]})"
                location_str = format_time_window(data["locations"][node], time_window)
    ...
    for vtype, entries in grouped_data.items():
        ...
        for v in entries:
            ...
            if v['times']:
                start_time = minutes_to_time_str(v['times']['start'])
                end_time = minutes_to_time_str(v['times']['end'])
