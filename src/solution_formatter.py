from src.utils import minutes_to_time_str, expand_vehicle_types, format_time_window
...
def format_solution_output(routing, manager, solution, solver_input, merged_data, config):
    ...
    for vehicle_id in range(routing.vehicles()):
        ...
        while not routing.IsEnd(index):
            ...
            if node != load_config()["depot_index"]:  # Skip depot
                location_id = solver_input["locations"][node]
                ...
                planned_time = minutes_to_time_str(time_min)
                ...
                stop_window = format_time_window(location_id, (stop_min, stop_max))
        ...
        if vehicle_stats[vehicle_id]["delivery_ids"]:  # Only if vehicle is used
            ...
            vehicle_stats[vehicle_id]["route"]["start_time"] = minutes_to_time_str(solution.Min(time_var_start))
            vehicle_stats[vehicle_id]["route"]["end_time"] = minutes_to_time_str(solution.Max(time_var_end))
